# pm-discover.ps1 - Create discovery session workspace for PM brainstorming and opportunity scoring
[CmdletBinding()]
param()
...
function Main {
    Write-LogInfo "Setting up PM discovery session..."

    # Generate session ID (timestamp-based)
    $sessionId = Get-Date -Format "yyyyMMdd-HHmmss"

    # Create discovery workspace directory
    $discoveryDir = ".gobuildme\specs\pm-discovery\$sessionId"
    New-Item -ItemType Directory -Path $discoveryDir -Force | Out-Null

    Write-LogSuccess "Created discovery workspace: $discoveryDir"

    # Create discovery.md from template
    New-DiscoveryTemplate -Dir $discoveryDir

    # Create opportunity-matrix.md
    New-OpportunityMatrix -Dir $discoveryDir

    # Create hypotheses.md for top problems
    New-HypothesesTemplate -Dir $discoveryDir

    # Create session metadata
    New-SessionMetadata -Dir $discoveryDir -SessionId $sessionId

    Write-LogInfo ""
    Write-LogSuccess "✅ PM Discovery session initialized!"
    Write-LogInfo ""
    Write-LogInfo "Session ID: $sessionId"
    Write-LogInfo "Workspace: $discoveryDir"
    Write-LogInfo ""
    Write-LogInfo "Files created:"
    Write-LogInfo "  - discovery.md           (Main discovery document)"
    Write-LogInfo "  - opportunity-matrix.md  (Scoring matrix)"
    Write-LogInfo "  - hypotheses.md          (Top 3 hypotheses)"
    Write-LogInfo "  - session-metadata.json  (Session info)"
    Write-LogInfo ""
    Write-LogInfo "Next: Follow the /gbm.pm.discover command to:"
    Write-LogInfo "  1. Explore 3-5 problems"
    Write-LogInfo "  2. Score opportunities"
    Write-LogInfo "  3. Form hypotheses"
    Write-LogInfo "  4. Choose direction"
}
...
function New-DiscoveryTemplate {
    param([string]$Dir)

    $file = Join-Path $Dir "discovery.md"
...
'@
    ...
    Write-LogSuccess "Created: discovery.md"
}

function New-OpportunityMatrix {
    param([string]$Dir)

    $file = Join-Path $Dir "opportunity-matrix.md"
...
'@
    ...
    Set-Content -Path $file -Value $content
    Write-LogSuccess "Created: opportunity-matrix.md"
}

function New-HypothesesTemplate {
    param([string]$Dir)

    $file = Join-Path $Dir "hypotheses.md"
...
'@
    ...
    Set-Content -Path $file -Value $content
    Write-LogSuccess "Created: hypotheses.md"
}

function New-SessionMetadata {
    param(
        [string]$Dir,
        [string]$SessionId
    )

    $file = Join-Path $Dir "session-metadata.json"
    ...
    Write-LogSuccess "Created: session-metadata.json"
}
